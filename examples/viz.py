...
FIGURE_SIZES = {
    ...
    "inference_scaling": (
        6,
        10,
    ),  # Two-panel scaling (slightly wider for shared x-axis)
...
FONT_HIERARCHY = {
    ...
    "axis_labels": 22,  # X/Y axis labels (bold)
...
def get_method_color(method_name: str) -> str:
    ...
    available_colors = (
        ...
        + list(IS_VARIANT_COLORS.keys())
        + list(SMC_METHOD_COLORS.keys())
    )
...
def setup_comparison_plot(
    figsize_key: str = "framework_comparison",
...
def finalize_comparison_plot(
    ax, xlabel: str, ylabel: str, title: Optional[str] = None, legend: bool = True
):
